...
function collect(cwd, options = {}) {
  ...
  data.readme = readFileIfExists(cwd, 'README.md')
    || readFileIfExists(cwd, 'README.adoc')
    || readFileIfExists(cwd, 'README.rst')
    || readFileIfExists(cwd, 'README');
...
function scanManifest(cwd) {
  ...
  const denoPath = fs.existsSync(denoJsonPath) ? denoJsonPath : (fs.existsSync(denoJsoncPath) ? denoJsoncPath : null);
  if (denoPath) {
    try {
      let content = fs.readFileSync(denoPath, 'utf8');
      // Strip control characters and trailing commas for JSONC compat
      // Don't strip // comments (they can appear in URLs inside strings)
      content = content.replace(/[\x00-\x1f\x7f]/g, ' ').replace(/,\s*([\]}])/g, '$1');
      const deno = JSON.parse(content);
      result.type = 'deno';
      result.language = 'typescript';
      result.name = deno.name;
      result.version = deno.version;
      result.scripts = deno.tasks ? Object.keys(deno.tasks) : [];
      if (deno.workspace) result.type = 'deno-workspace';
      return result;
    } catch { /* parse error */ }
  }
  ...
  // meson.build (C/C++, Nix)
  if (fs.existsSync(path.join(cwd, 'meson.build'))) {
    result.type = 'meson';
    result.language = 'c/c++';
    try {
      const meson = fs.readFileSync(path.join(cwd, 'meson.build'), 'utf8');
      const projMatch = meson.match(/project\s*\(\s*'([^']+)'/);
      if (projMatch) result.name = projMatch[1];
      const verMatch = meson.match(/version\s*:\s*'([^']+)'/);
      if (verMatch) result.version = verMatch[1];
    } catch { /* ignore */ }
    return result;
  }
...
function scanStructure(cwd, maxDepth = 3, maxEntries = 60) {
  const tree = [];
  ...
  walkDir(cwd, '', 0, maxDepth, tree, maxEntries);

  // Cap total entries to avoid token bloat on huge monorepos
  if (tree.length > maxEntries) {
    tree.length = maxEntries;
    tree.push({ path: '[... truncated]', depth: 0, files: 0, dirs: 0 });
  }

  return tree;
}

function walkDir(basePath, relPath, depth, maxDepth, result, maxEntries) {
  if (depth > maxDepth) return;
  if (maxEntries && result.length >= maxEntries) return;
  ...
  for (const dir of dirs.sort()) {
    walkDir(basePath, relPath ? relPath + '/' + dir : dir, depth + 1, maxDepth, result, maxEntries);
  }
}
...
function getContributorSignals(cwd) {
  try {
    ...
    // can-i-help query: good-first areas, areas needing help
    try {
      signals.canIHelp = JSON.parse(
        binary.runAnalyzer(['repo-intel', 'query', 'can-i-help', '--map-file', mapFile, cwd])
      );
    } catch { /* query not available */ }

    // test-gaps: hot source files without co-changing test files
    try {
      signals.testGaps = JSON.parse(
        binary.runAnalyzer(['repo-intel', 'query', 'test-gaps', '--map-file', mapFile, cwd])
      );
    } catch { /* query not available */ }

    // doc-drift: stale documentation
    try {
      signals.docDrift = JSON.parse(
        binary.runAnalyzer(['repo-intel', 'query', 'doc-drift', '--map-file', mapFile, cwd])
      );
    } catch { /* query not available */ }

    // bugspots: bug-prone files
    try {
      signals.bugspots = JSON.parse(
        binary.runAnalyzer(['repo-intel', 'query', 'bugspots', '--map-file', mapFile, cwd])
      );
    } catch { /* query not available */ }
...
function getOpenIssues(cwd) {
  try {
    const output = cp.execFileSync('gh', [
      ...
      '--limit', '15', '--json', 'number,title,labels,assignees'
    ], {
      cwd,
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe'],
