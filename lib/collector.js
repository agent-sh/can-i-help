...
/**
 * Can-I-Help data collector
 *
 * Gathers project context plus contributor-specific signals via pure JS (no LLM calls).
 * Produces a single data object that the can-i-help agent reads.
 *
...
function collect(cwd, options = {}) {
  ...
  data.contributorSignals = getContributorSignals(cwd);
  data.issues = getOpenIssues(cwd);
...
// Takes an explicit argv array (not a string) so future multi-word args
// like `--format=%H %s` can't silently break via `.split(' ')`.
...
function getRepoIntel(cwd) {
  try {
    let binary;
    try { binary = require('./agentsys').get().binary; } catch { return null; }
    ...
    // Generate if missing. Probe with an open-for-read (not existsSync, which
    // is the TOCTOU check CodeQL flags) and publish via a unique temp file +
    // atomic rename, so the cache becomes visible only once fully written - a
    // concurrent reader never observes a partial/empty file.
    let haveCache = true;
    try {
      fs.closeSync(fs.openSync(mapFile, 'r'));
    } catch {
      haveCache = false;
    }
    if (!haveCache) {
      fs.mkdirSync(path.join(cwd, stateDir), { recursive: true });
      const tempFile = `${mapFile}.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}.tmp`;
      try {
        const output = binary.runAnalyzer(['repo-intel', 'init', cwd]);
        JSON.parse(output);
        fs.writeFileSync(tempFile, output);
        fs.renameSync(tempFile, mapFile);
      } catch (e) {
...
function getRepoMap(cwd) {
  try {
    ...
    return summarizeRepoMapFiles(map.files);
  } catch {
    return null;
  }
}

// Max files in keyExports, to keep the data file small.
const MAX_KEY_EXPORT_FILES = 20;

// agentsys writes files[path].symbols = { exports, functions, classes, types,
// constants }; older maps used a flat array of { name, exported }.
function summarizeRepoMapFiles(files) {
  const totalSymbols = Object.values(files).reduce((sum, f) => sum + symbolCount(f && f.symbols), 0);
  const keyExports = {};
  Object.entries(files)
    .map(([filePath, fd]) => [filePath, exportedNames(fd && fd.symbols)])
    .filter(([, names]) => names.length > 3)
    .sort((a, b) => b[1].length - a[1].length)
    .slice(0, MAX_KEY_EXPORT_FILES)
    .forEach(([filePath, names]) => { keyExports[filePath] = names.slice(0, 15); });
  return { totalFiles: Object.keys(files).length, totalSymbols, keyExports };
}

function exportedNames(symbols) {
  if (Array.isArray(symbols)) return symbols.filter(s => s && s.exported).map(s => s.name);
  if (symbols && Array.isArray(symbols.exports)) return symbols.exports.map(s => s && s.name).filter(Boolean);
  return [];
}
...
function symbolCount(symbols) {
  ...
  // An exported function can appear under both exports and functions.
  const seen = new Set();
  for (const list of Object.values(symbols)) {
    if (!Array.isArray(list)) continue;
    for (const sym of list) seen.add(`${sym && sym.name}:${sym && sym.line}`);
  }
  return seen.size;
}
...
// Slop categories that make natural first contributions. Orphan exports and
// commented-out code are pure deletions; passthrough wrappers need call sites
// updated; always-true conditions usually point at a real bug.
...
const SLOP_SAMPLE_CAP = 10;

function partitionSlop(raw) {
  const fixes = Array.isArray(raw) ? raw : (raw && Array.isArray(raw.fixes) ? raw.fixes : []);
  const out = { counts: {} };
  for (const key of Object.values(SLOP_CATEGORIES)) {
    out[key] = [];
    out.counts[key] = 0;
  }
  for (const fix of fixes) {
    const key = SLOP_CATEGORIES[fix && fix.category];
    if (!key) continue;
    out.counts[key] += 1;
    if (out[key].length < SLOP_SAMPLE_CAP) out[key].push(fix);
  }
  return out;
}

function getContributorSignals(cwd) {
  try {
    let binary;
    try { binary = require('./agentsys').get().binary; } catch { return null; }
    ...
    const q = (args) => {
      try {
        return JSON.parse(binary.runAnalyzer(['repo-intel', 'query', ...args, '--map-file', mapFile, cwd]));
      } catch {
        return null; // query not available in this analyzer version
      }
    };

    const signals = {
      canIHelp: q(['can-i-help']),
      testGaps: q(['test-gaps', '--top', '15']),
      docDrift: q(['doc-drift', '--top', '10']),
      bugspots: q(['bugspots', '--top', '10']),
      staleDocs: q(['stale-docs', '--top', '10']),
      conventions: q(['conventions'])
    };
    const slopRaw = q(['slop-fixes']);
    signals.slopFirstContributions = slopRaw === null ? null : partitionSlop(slopRaw);
...
function getOpenIssues(cwd) {
  try {
    const output = cp.execFileSync('gh', [
      'issue', 'list', '--state', 'open',
      '--limit', '15', '--json', 'number,title,labels,assignees,createdAt'
    ], {
      cwd,
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe'],
...
module.exports = {
  ...
  getRepoIntel,
  getRepoMap,
  getContributorSignals,
  getOpenIssues,
  partitionSlop,
  summarizeRepoMapFiles
};
