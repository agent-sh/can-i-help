...
function getRepoIntel(cwd) {
  try {
    ...
    // Generate if missing. Claim the cache file with an exclusive create
    // (`wx`): if it already exists, a concurrent run produced it - reuse it
    // and skip generation. This removes the exists-then-write TOCTOU race
    // (no separate existence check feeding an unconditional overwrite).
    fs.mkdirSync(path.join(cwd, stateDir), { recursive: true });
    let mapFd = null;
    try {
      mapFd = fs.openSync(mapFile, 'wx');
    } catch (e) {
    ...
    if (mapFd !== null) {
      try {
        ...
        JSON.parse(output);
        fs.writeSync(mapFd, output);
      } catch (e) {
        fs.closeSync(mapFd);
        try { fs.unlinkSync(mapFile); } catch { /* best-effort cleanup */ }
        throw e;
      }
      fs.closeSync(mapFd);
    }
